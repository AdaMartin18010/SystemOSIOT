...
- **定理2.1**：在分布式数据存储系统中，若发生网络分区，则无法同时满足线性一致性（C）与可用性（A）。由于真实分布式系统必须容忍分区（P），因此设计时只能选择 CP 或 AP。
  Theorem 2.1: In a distributed data store, if a network partition occurs, the system cannot simultaneously guarantee linearizability (C) and availability (A). Since partition tolerance (P) is mandatory in real distributed systems, the design choice is between CP and AP.
- **证明**：
  - 假设系统在分区发生时同时满足 C、A、P
  - 网络分区发生时，节点间无法通信
  - 为满足 A，系统必须响应请求
  - 为满足 P，系统必须在分区下继续运行
  - 但无法满足 C（线性一致性），因为分区两侧节点状态将不一致
  - 矛盾，故分区发生时必须在 C 与 A 之间取舍
