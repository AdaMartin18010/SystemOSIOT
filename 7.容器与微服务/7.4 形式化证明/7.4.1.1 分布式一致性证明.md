...
> **关于 CA 的说明**：在真实分布式系统中，P（分区容忍性）不可避免，因此不存在“放弃 P”的 CA 分布式系统。以下 CA 形式化仅适用于单机或非分布式场景；分布式系统分类应为 CP/AP。

**CP系统（分区时放弃A）：**
$$System_{CP} = (Consistency, \neg Availability_{under\_partition}, PartitionTolerance)$$

**AP系统（分区时放弃C）：**
$$System_{AP} = (\neg Consistency_{under\_partition}, Availability, PartitionTolerance)$$

**PACELC扩展**：
$$\text{If Partition} \implies (C \lor A) \land \neg(C \land A)$$
$$\text{Else} \implies (L \lor C) \land \neg(L \land C)$$
