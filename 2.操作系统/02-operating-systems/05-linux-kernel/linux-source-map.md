...
- [Linux 内核源码与子系统映射](#linux-内核源码与子系统映射)
  - [1. 内核源码顶层目录映射](#1-内核源码顶层目录映射)
  - [2. 进程管理实现映射](#2-进程管理实现映射)
    - [2.1 核心数据结构](#21-核心数据结构)
    - [2.2 关键函数与系统调用](#22-关键函数与系统调用)
    - [2.3 调度类层次](#23-调度类层次)
  - [3. 内存管理实现映射](#3-内存管理实现映射)
    - [3.1 核心数据结构](#31-核心数据结构)
    - [3.2 关键函数与算法](#32-关键函数与算法)
    - [3.3 地址空间布局（x86-64）](#33-地址空间布局x86-64)
  - [4. 文件系统实现映射](#4-文件系统实现映射)
    - [4.1 VFS 核心数据结构](#41-vfs-核心数据结构)
    - [4.2 关键函数与系统调用](#42-关键函数与系统调用)
    - [4.3 VFS 调用链](#43-vfs-调用链)
  - [5. 设备管理实现映射](#5-设备管理实现映射)
    - [5.1 Linux 设备模型](#51-linux-设备模型)
    - [5.2 关键源码](#52-关键源码)
    - [5.3 设备树映射](#53-设备树映射)
  - [6. 网络子系统实现映射](#6-网络子系统实现映射)
  - [7. 安全子系统实现映射](#7-安全子系统实现映射)
  - [8. 启动流程映射](#8-启动流程映射)
  - [9. 术语表](#9-术语表)
  - [10. 国际来源映射](#10-国际来源映射)
