...
- [3.3.1 形式化定义](#331-形式化定义)
  - [1. 基本形式化定义](#1-基本形式化定义)
    - [1.1 物联网嵌入式系统形式化定义](#11-物联网嵌入式系统形式化定义)
    - [1.2 设备形式化定义](#12-设备形式化定义)
    - [1.3 网络形式化定义](#13-网络形式化定义)
  - [2. 系统架构形式化定义](#2-系统架构形式化定义)
    - [2.1 分层架构定义](#21-分层架构定义)
    - [2.2 边缘计算架构定义](#22-边缘计算架构定义)
  - [3. 协议与通信形式化定义](#3-协议与通信形式化定义)
    - [3.1 协议栈定义](#31-协议栈定义)
    - [3.2 通信模式定义](#32-通信模式定义)
  - [4. 数据处理形式化定义](#4-数据处理形式化定义)
    - [4.1 数据流定义](#41-数据流定义)
    - [4.2 数据存储定义](#42-数据存储定义)
  - [5. 安全与隐私形式化定义](#5-安全与隐私形式化定义)
    - [5.1 安全机制定义](#51-安全机制定义)
    - [5.2 隐私保护定义](#52-隐私保护定义)
  - [6. 性能与可靠性形式化定义](#6-性能与可靠性形式化定义)
    - [6.1 性能指标定义](#61-性能指标定义)
    - [6.2 可靠性定义](#62-可靠性定义)
  - [7. 结构化表达](#7-结构化表达)
    - [7.1 系统分类表](#71-系统分类表)
    - [7.2 架构关系图](#72-架构关系图)
    - [7.3 性能指标矩阵](#73-性能指标矩阵)
  - [8. 多表征](#8-多表征)
    - [8.1 数学符号表达](#81-数学符号表达)
    - [8.2 结构化表达](#82-结构化表达)
  - [9. 规范说明](#9-规范说明)
