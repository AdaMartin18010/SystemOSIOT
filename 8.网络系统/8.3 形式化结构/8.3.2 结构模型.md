...
- 中文：
  - **OSI七层模型**：[ISO/IEC 7498-1](https://www.iso.org/standard/20269.html)标准定义的七层网络架构模型，包括物理层、数据链路层、网络层、传输层、会话层、表示层、应用层。
  - **TCP/IP四层模型**：[RFC 1122](https://datatracker.ietf.org/doc/html/rfc1122)定义的网络架构模型，包括网络接口层、网络层、传输层、应用层。
  - **SDN架构模型**：软件定义网络的层次化架构模型，包括数据平面、控制平面、应用平面。

- EN:
  - **OSI Seven-layer Model**: Seven-layer network architecture model defined by [ISO/IEC 7498-1](https://www.iso.org/standard/20269.html) standard, including physical layer, data link layer, network layer, transport layer, session layer, presentation layer, and application layer.
  - **TCP/IP Four-layer Model**: Network architecture model defined by [RFC 1122](https://datatracker.ietf.org/doc/html/rfc1122), including network interface layer, network layer, transport layer, and application layer.
...
- 中文：
  - **IEEE 802标准系列**：[IEEE 802.3](https://standards.ieee.org/standard/802.3-2022.html)（以太网）、IEEE 802.11（无线局域网）、IEEE 802.15（无线个人区域网）等标准的结构模型。
  - **[3GPP](https://www.3gpp.org/specifications)标准系列**：3G、4G、5G移动通信网络的结构模型，包括RAN、CN、IMS等组件。
  - **IETF RFC标准**：互联网协议栈的结构模型，包括IPv4、IPv6、TCP、UDP等协议的结构定义。

- EN:
  - **IEEE 802 Standards Series**: Structural models of standards such as [IEEE 802.3](https://standards.ieee.org/standard/802.3-2022.html) (Ethernet), IEEE 802.11 (Wireless LAN), and IEEE 802.15 (Wireless Personal Area Network).
  - **[3GPP](https://www.3gpp.org/specifications) Standards Series**: Structural models of 3G, 4G, 5G mobile communication networks, including components such as RAN, CN, and IMS.
