...
- 中文：
  - **网络拓扑图**：[ISO/IEC 7498-1](https://www.iso.org/standard/20269.html)标准定义的网络拓扑结构图，包括节点、链路、接口等要素。
  - **网络架构图**：[RFC 1122](https://datatracker.ietf.org/doc/html/rfc1122)定义的网络架构层次图，包括物理层、网络层、传输层、应用层。
  - **网络部署图**：IEEE 802标准定义的网络部署结构图，包括设备布局、连接关系、配置参数。

- EN:
  - **Network Topology Diagram**: Network topology structure diagram defined by [ISO/IEC 7498-1](https://www.iso.org/standard/20269.html) standard, including elements such as nodes, links, and interfaces.
  - **Network Architecture Diagram**: Network architecture hierarchy diagram defined by [RFC 1122](https://datatracker.ietf.org/doc/html/rfc1122), including physical layer, network layer, transport layer, and application layer.
...
- 中文：
  - **OSI协议栈表**：[ISO/IEC 7498-1](https://www.iso.org/standard/20269.html)定义的七层协议栈对照表，包括层号、层名、功能、协议等。
  - **TCP/IP协议栈表**：[RFC 1122](https://datatracker.ietf.org/doc/html/rfc1122)定义的协议栈对照表，包括协议名称、端口号、功能描述等。
  - **安全协议栈表**：IETF RFC定义的安全协议栈表，包括加密算法、认证方式、密钥管理等。

- EN:
  - **OSI Protocol Stack Table**: Seven-layer protocol stack comparison table defined by [ISO/IEC 7498-1](https://www.iso.org/standard/20269.html), including layer number, layer name, functions, and protocols.
  - **TCP/IP Protocol Stack Table**: Protocol stack comparison table defined by [RFC 1122](https://datatracker.ietf.org/doc/html/rfc1122), including protocol names, port numbers, and function descriptions.
...
- 中文：
  - **[3GPP](https://www.3gpp.org/specifications)网络架构图**：[3GPP](https://www.3gpp.org/specifications)标准定义的移动通信网络架构图，包括RAN、CN、IMS等组件。
  - **IEEE 802网络图**：IEEE 802标准定义的局域网架构图，包括以太网、无线局域网等。
  - **IETF网络图**：IETF RFC定义的互联网架构图，包括路由器、交换机、服务器等。

- EN:
  - **[3GPP](https://www.3gpp.org/specifications) Network Architecture Diagram**: Mobile communication network architecture diagram defined by [3GPP](https://www.3gpp.org/specifications) standards, including components such as RAN, CN, and IMS.
