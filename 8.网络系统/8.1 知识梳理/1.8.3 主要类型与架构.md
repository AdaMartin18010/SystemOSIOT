...
- 中文：
  ...
  - **国际标准**：IEEE 802系列（以太网、WiFi）、[3GPP](https://www.3gpp.org/specifications)（5G）、IETF（TCP/IP、HTTP）、ISO/IEC（OSI模型）等。
- EN:
  ...
  - **International Standards**: IEEE 802 series (Ethernet, WiFi), [3GPP](https://www.3gpp.org/specifications) (5G), IETF (TCP/IP, HTTP), ISO/IEC (OSI model), etc.
