...
**定义4.3.1.13（CAP定理）**：在分布式数据存储系统中，若发生网络分区（$\text{Partition}$ 为真），则无法同时满足线性一致性（Consistency）与可用性（Availability）。由于真实分布式系统必须满足分区容忍性 P，因此 CAP 的实际形式化表述为：
...
**形式化表述**：
$$\text{Partition} \implies \neg (\text{Consistency} \land \text{Availability})$$

即：分区发生时，C 与 A 不可兼得。P 在分布式系统中不可放弃，因此系统分类为 CP 或 AP。

**PACELC 扩展**：
$$\text{Partition} \implies (C \lor A) \land \neg(C \land A)$$
$$\neg \text{Partition} \implies (L \lor C) \land \neg(L \land C)$$
