...
**定理4.1.1.1（CAP定理）**：在分布式数据存储系统中，若发生网络分区，则无法同时满足线性一致性（Consistency）与可用性（Availability）。由于真实分布式系统无法避免网络分区，因此设计时必须在分区发生时选择保证一致性（CP）或可用性（AP）。

**证明**：

1. 假设系统在分区发生时同时满足 C、A、P。
2. 发生网络分区，将节点分为两个集合 P₁, P₂。
3. 客户端向 P₁ 写入数据 d₁，向 P₂ 写入数据 d₂。
4. 由于 P（分区容忍性），两个分区继续运行。
5. 由于 A（可用性），两个分区都响应请求。
6. 由于 C（线性一致性），所有节点应看到相同数据。
7. 矛盾：P₁ 和 P₂ 看到不同数据，违反 C。
8. 因此，在分区发生时，系统必须在 C 与 A 之间取舍；P 在真实分布式系统中不可放弃。

> 参考：Gilbert & Lynch, *Brewer's Conjecture and the Feasibility of Consistent, Available, Partition-Tolerant Web Services*, SIGACT News 2002. DOI: 10.1145/564585.564601
